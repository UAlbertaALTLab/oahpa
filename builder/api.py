...
from sms_drill.models import ( Word, Form, Semtype, Tag, Tagset, Tagname )
...
class WordView(viewsets.ModelViewSet):
    ...
    search_fields = ('=lemma', '=pos')

    def list(self, request):
        """ GET request.
        """

        rq = super(WordView, self).list(request)
    ...
    def filter_queryset(self, queryset):
        ...
        ps = {
            ...
            'form__tag__string': self.request.QUERY_PARAMS.get('tag', None),
        }
        if semtypes:
    ...
    def metadata(self, request):
        ...
        data = super(WordView, self).metadata(request)
        poses = list(set([
            t.tagname for t in Tagname.objects.filter(tagset__tagset='Wordclass')
        ]))

        vals = {
            'note': "Not for validation purposes.",
            'semtypes': Semtype.objects.all().values_list('semtype', flat=True),
            'pos': poses,
            'tags': Tag.objects.values_list('string', flat=True)
        }
        data['possible_values'] = vals
...
class FormView(viewsets.ModelViewSet):
    ...
    # filter_backends = (filters.SearchFilter,)
    # search_fields = ('fullform', 'tag__string', 'word__lemma')
    ...
    def filter_queryset(self, queryset):
        ...
        """
        ps = {
            'fullform': self.request.QUERY_PARAMS.get('fullform', None),
            'tag__string': self.request.QUERY_PARAMS.get('tag', None),
