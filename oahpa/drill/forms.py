...
def is_correct(self, game, example=None):
    ...
    log = Log.objects.create(userinput=answer,
                                correct=correctlist,
                                iscorrect=iscorrect,
                                example=example,
                                game=game,
...
class QuizzSettings(OahpaSettings):
    ...
    semtype = forms.ChoiceField(initial='FAMILY', choices=SEMTYPE_CHOICES, widget=forms.Select(attrs={'onchange':'javascript:return SetIndex(document.gameform.book,this.value);',}))
