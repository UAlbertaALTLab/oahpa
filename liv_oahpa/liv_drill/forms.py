...
import liv_oahpa.settings as settings

from liv_oahpa.conf.tools import switch_language_code

from .models import *
#from game import *
#from liv_oahpa.liv_drill.game import relax
import datetime
import socket
import sys, os
import itertools
...
PRONOUNS_LIST = {'Sg1':'ma', 'Sg2':'sa', 'Sg3':'ta',
		  'Pl1':'mēg', 'Pl2':'tēg', 'Pl3':'ne'}

# DEMONSTRATIVE_PRESENTATION plus Sg3/Pl3
PASSIVE_PRONOUNS_LIST = {'Sg1':'ma', 'Sg2':'sa', 'Sg3':'ta',
                         'Pl1':'mēg', 'Pl2':'tēg', 'Pl3':'ne'}
...
NOUN_TYPE_CHOICES =(
	('N-FEM-other', _('feminine in -a/-я')),
	('N-FEM-8', _('feminine in -ь')),
...
from liv_oahpa.settings import INFINITIVE_SUBTRACT as infinitives_sub
from liv_oahpa.settings import INFINITIVE_ADD as infinitives_add
...
class OahpaSettings(forms.Form):
	...
	def set_default_data(self):
		self.default_data = {
					'language' : 'liv',  # sme in univ_oahpa
...
class OahpaQuestion(forms.Form):
	...
	def init_variables(self, possible, userans_val, accepted_answers, preferred=False):
		...
		if hasattr(self, 'translang'): # commented out these two lines, because otherwise relax was not working in Morfa
			if self.translang == 'liv': # caused a problem in Numra, as NumQuestion does not have the attribute translang 
				# Relax spellings.
...
class LeksaSettings(OahpaSettings):
	...
	default_data = {'gametype' : 'leksa', 'language' : 'liv', 'dialogue' : 'GG',
			#'syll' : [],
			#'bisyllabic': False,
			#'trisyllabic': False,
			#'bisyllabic': False,
...
class NumSettings(OahpaSettings):
	...
	default_data = {'language' : 'liv', 'numlanguage' : 'liv', 'dialogue' : 'GG', 'maxnum' : '10', 'numgame': 'numeral'}
...
class KlokkaSettings(NumSettings):
	...
	default_data = {'language' : 'liv', 'numlanguage' : 'liv', 'dialogue' : 'GG', 'gametype' : 'kl1', 'numgame': 'string'}
...
class DatoSettings(KlokkaSettings):
	...
	default_data = {'language' : 'liv', 'numlanguage' : 'liv', 'numgame': 'string'}
...
def vasta_is_correct(self,question,qwords,language,utterance_name=None):
    ...
    #logfile = open('/home/liv_oahpa/liv_oahpa/liv_drill/vastaF_log.txt','w')
...
def cealkka_is_correct(self,question,qwords,awords,language,question_id=None):  #was: question_id=None
    ...
    #logfile = open('/home/liv_oahpa/liv_oahpa/liv_drill/vastas_log.txt', 'w')
    host = 'localhost'
    port = 9000  # was: 9000, TODO - add to settings.py
    size = 1024
