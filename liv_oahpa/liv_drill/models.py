...
class BulkManager(models.Manager):
	...
	def bulk_add_form_messages(self, objs):
		...
		sql = "INSERT %s INTO %s (%s) VALUES %s" % (ignore, "liv_drill_form_feedback", flds, arg_string,)
	...
	def bulk_add_messages(self, objs):
		...
		arg_string = ', '.join(['(' + ', '.join(['%s']*len(fields)) + ')'] * len(vals))
		sql = "INSERT INTO %s (%s) VALUES %s" % ("liv_drill_feedback_messages", flds, arg_string,)
	...
	def bulk_add_dialects(self, objs):
		...
		arg_string = ', '.join(['(' + ', '.join(['%s']*len(fields)) + ')'] * len(vals))
		sql = "INSERT INTO %s (%s) VALUES %s" % ("liv_drill_feedback_dialects", flds, arg_string,)
...
def Translations2(target_lang):
	if target_lang in ["nob", "liv", "eng", "dan", "no"]:
		if target_lang == "nob" or "no":	related = 'translations2nob'
		if target_lang == "liv":	related = 'translations2liv'
...
class Word(models.Model):
	...
	language = models.CharField(max_length=5, default='liv', db_index=True)
	...
	def __init__(self, *args, **kwargs):
		...
		self.translations2liv = partial(self.translations2, target_lang='liv')()
