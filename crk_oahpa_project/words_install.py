...
try:
    ...
    print(
        """Dialects not defined in settings.py...
		DIALECTS = {
...
try:
    ...
    print(
        """Infinitives not defined in settings.py...
			INFINITIVE_SUBTRACT = {
...
class Analysis(object):
    ...
    def getTag(self):
        ...
        tag_kwargs = {
            ...
            "preverb": self.classes.get("Preverb", ""),
            "object": self.classes.get("Object", ""),
            "polarity": self.classes.get("Polarity", ""),
            "tense": self.classes.get("Tense", ""),
            "intentional_definite": self.classes.get("Intentional-Definite", ""),
    ...
    def processLG(self):
        ...
        l_attrs = [
            ...
            (
                "object",
                None,
            ),  # TODO: maybe need to relabel this v_object if python doesn't like object attribute
            ("audio", None),
            ("gradation", None),
            ("diphthong", None),
            ("rime", None),
...
class Words(object):
    ...
    def install_lexicon(
            ...
            if entry.lemma_analyses or entry.analysis:
                ...
                if paradigmfile:
                    ...
                    try:
                        ...
                        print(e.toxml())
                        print(
                            "XML file contains an empty <l /> element.", file=sys.stderr
                        )
        ...
        for entry in entries:
            ...
            if pos:
                ...
                if not COUNT_ONLY:
                    if VERBOSE:
                        print(
                            "undefined pos for ", __data.encode("utf-8"), file=_STDOUT
                        )
        ...
        if delete and pos:
            allids = (
                sdm.Word.objects.filter(pos=pos)
            ...
            for a in allids:
                if force_str(a) not in set(self.all_wordids):
                    print(
                        "Word id not found from xml. Deleting:",
                        a.encode("utf-8"),
                        file=_STDOUT,
                    )
    ...
    def add_translation(self, language, txdata, w, entry, semantics):
        ...
        if not translation:
            print(
                " *** No translation lemma given in word translation elements for <%s>. Skipping this translation."
        ...
        try:
            ...
            print(
                "Extra similar translation objects found, deleting extras...",
                file=_STDERR,
            )
        ...
        if VERBOSE:
            print(
                "Translation for <%s> added: %s"
                % (language.encode("utf-8"), translation.encode("utf-8")),
                file=_STDOUT,
            )
    ...
    def add_semantics(self, semantics, w, entry):
        if entry.exclude:
            ...
            for exclusion in exclusions:
                exclude_type, _ = sdm.Semtype.objects.get_or_create(
                    semtype="exclude_" + exclusion
                )
                w.semtype.add(exclude_type)
                direction = (exclusion[0:3], exclusion[3:6])
                if VERBOSE:
                    print(
                        " *** This word will be excluded in %s->%s" % direction,
                        file=_STDOUT,
                    )
        ...
        if semantics:
            for semclass in semantics:
                ...
                if created:
                    if VERBOSE:
                        print(
                            "Created semtype entry with name ",
                            semclass.encode("utf-8"),
                            file=_STDOUT,
                        )
    ...
    def add_chapters(self, entry, w):
        for chapter in entry.chapters:
            if VERBOSE:
                print("Added chapter ", chapter.encode("utf-8"), file=_STDOUT)
            w.chapter = chapter
            w.save()

    def add_dialects(self, entry, w):
        for dialect, dial_data in list(
            DIALECTS.items()
        ):  # fill the dialect field of the Word object - added by Heli
            dial, created = sdm.Dialect.objects.get_or_create(dialect=dialect)
            if created:
                if VERBOSE:
                    print(
                        "Added dialects to Word ", dialect.encode("utf-8"), file=_STDOUT
                    )
    ...
    def store_word(
        self, entry, linginfo, mainlang, paradigmfile, delete, append_only=False
    ):
        OUT_STRS = []
        ERR_STRS = []
        ...
        if stem in trisyllabic:
            stem = "3syll"
        if stem in bisyllabic:
            stem = "2syll"
        ...
        if pregenerated:
            ...
            if entry.lemma_analyses:
                ...
                # Join tags
                analyses = [
                    (form, "+".join(tags), dial) for form, tags, dial in analyses
                ]
            ...
            for analysis in analyses:
                ...
                form, _ = sdm.Form.objects.get_or_create(
                    fullform=g.form, tag=tag, word=w
                )
                    ...
                    if dialect:
                        if VERBOSE:
                            OUT_STRS.append(
                                force_str(
                                    "Created form: %s\t%s\t\t%s"
                                    % (tag.string, g.form, dialect.dialect)
                                )
                            )
                    else:
                        if VERBOSE:
                            OUT_STRS.append(
                                force_str("Created form: %s\t%s" % (tag.string, g.form))
                            )
        elif paradigmfile:
            ...
            for dialect in dialect_objects:
                ...
                generated_forms = linginfo.get_paradigm(
                    lemma=lemma,
                    pos=pos,
                    forms=forms,
            ...
            paradigms_to_create = sco.OrderedDict(
                sorted(list(paradigms_to_create.items()), key=lambda t: t[0])
            )

            changes_to_paradigm = False
            paradigm_key = "%s|%s|%s" % (lemma, pos, dialect.dialect)
            changes_to_paradigm = self.paradigm_is_changed(
                paradigm_key, paradigms_to_create
            )
            ...
            if changes_to_paradigm:
                ...
                for key, item in paradigms_to_create.items():
                    ...
                    if (
                        w.pos == "A"
                        and w.compare == "no"
                        and (g.get("Grade") == "Comp" or g.get("Grade") == "Superl")
                    ):
                        continue
                    ...
                    if not COUNT_ONLY:
                        if VERBOSE:
                            fmt = (t.string, f.form, ", ".join(list(names)))
        ...
        if not changes_to_paradigm:
            OUT_STRS.append(
                " * No changes in generation detected, skipping Form objects."
            )
