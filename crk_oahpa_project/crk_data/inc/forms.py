...
GEOGRAPHY_CHOICES = (
    ...
    (
        "sapmi",
        _("sapmi"),
    ),  # was: sápmi, maybe characters with diacritics not allowed in drop-down menus
    ("suopma", _("suopma")),
)
        ...
        **kwargs
        ...
        if type(word) == Word:
            if self.sourcelang == "rus":
                self.lemma = (
                    word.lemma_stressed
                )  # for Rusian the words will be presented with stress marks
            else:
                self.lemma = (
                    word.lemma
        ...
        if word.pos.upper() == "V":
            if self.translang in infinitives_sub and infinitives_add:
                ...
                self.correct_ans = [
                    lemma.sub(infin_a, force_str(ax)) for ax in self.correct_ans
                ]
...
class MorfaSettings(OahpaSettings):
    ...
    vtype_context = forms.ChoiceField(
        initial="V-PRS", choices=VTYPE_CONTEXT_CHOICES, widget=forms.Select
    )
    pron_context = forms.ChoiceField(
        initial="P-PERS", choices=PRON_CONTEXT_CHOICES, widget=forms.Select
    )
        ...
        **kwargs
        ...
        if answer_tag_el.pos == "N":
            if qtype == "COLL-NUM":
                ...
                if (
                    answer_tag_el.number == "Sg"
                    or answer_tag_el.case == "Ess"
                    or answer_tag_el.case == "Nom"
                ):  # was: qtype="N-NOM-PL"
                    self.lemma = answer_word_el.lemma
                else:
                    nplforms = Form.objects.filter(
                        word__pk=answer_word, tag__string="N+Pl+Nom"
                    )
                    if nplforms.count() > 0:
        ...
        try:
            answer_word_form = Form.objects.exclude(dialects__dialect="NG").filter(
                word__pk=answer_word, tag=answer_tag_el, dialects__dialect=self.dialect
            )
