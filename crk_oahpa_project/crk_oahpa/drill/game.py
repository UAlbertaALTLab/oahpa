...
class Game(object):
    def __init__(self, settings):
        ...
        if (
            self.settings["gametype"] == "bare"
            and "pron_type" in self.settings
            and self.settings["pron_type"] in ["Rel", "Dem"]
        ):
            self.num_fields = 4
    ...
    def new_game(self):
        ...
        while i < self.num_fields and len(self.form_list) < 5 and tries < maxtries:
            ...
            if word_id:
                ...
                if word_id in set(
                    word_ids
                ):  # and not (self.settings['gametype'] == "bare" and self.settings['pron_type'] in ['Rel','Dem']): # If there are less than 5 different lemmas to choose from then this causes a "No questions were able to be generated."
                    continue
                else:
    ...
    def check_game(self, data=None):
        ...
        question_wordObj = re.compile(
            r"^question_word_(?P<syntaxString>[\w\-]*)$", re.U
        )
        question_fullformObj = re.compile(
            r"^question_fullform_(?P<syntaxString>[\w\-]*)$", re.U
        )
        ...
        answer_fullformObj = re.compile(
            r"^answer_fullform_(?P<syntaxString>[\w\-]*)$", re.U
        )

        answer_taskwordObj = re.compile(
            r"^answer_taskword_(?P<syntaxString>[\w\-]*)$", re.U
        )  # added by Heli
        ...
        for n in range(1, self.num_fields):
            ...
            for fieldname, value in list(data.items()):
                ...
                if fieldname.count(str(n) + "-") > 0:
                    ...
                    qwords = self.search_info(
                        question_tagObj, fieldname, value, qwords, "tag"
                    )
                    qwords = self.search_info(
                        question_wordObj, fieldname, value, qwords, "word"
                    )
                    qwords = self.search_info(
                        question_fullformObj, fieldname, value, qwords, "fullform"
                    )

                    tmpawords = self.search_info(
                        answer_tagObj, fieldname, value, tmpawords, "tag"
                    )
                    tmpawords = self.search_info(
                        answer_wordObj, fieldname, value, tmpawords, "word"
                    )
                    tmpawords = self.search_info(
                        answer_fullformObj, fieldname, value, tmpawords, "fullform"
                    )
                    tmpawords = self.search_info(
                        answer_taskwordObj, fieldname, value, tmpawords, "taskword"
                    )  # added by Heli

                    self.global_targets = self.search_info(
                        targetObj, fieldname, value, self.global_targets, "target"
                    )
    ...
    def get_score(self, data):
        ...
        if (self.show_correct or self.all_correct) and not self.settings[
            "gametype"
        ] == "qa":
            if i == 2:
                i = 3
            ...
            if "language" in self.settings:
                ...
                com_count = Comment.objects.filter(
                    Q(level=i) & Q(lang=language)
                ).count()
                if com_count > 0:
                    self.comment = Comment.objects.filter(
                        Q(level=i) & Q(lang=language)
                    )[randint(0, com_count - 1)].comment
...
class BareGame(Game):
    ...
    def get_db_info(self, db_info):
        ...
        singular_only = self.settings.get(
            "singular_only", False
        )  # make it possible to only generate singular exercises if the user wishes so
        ...
        num_type = (
            True and self.settings.get("num_type") or ""
        )  # added to get num_type from settings
        ...
        if pos == "V":
            TAG_EXCLUDES = Q(
                personnumber__contains="4"
            )  # persons 4Sg and 4Pl are excluded
        else:
        ...
        def parse_tag(tag):
            ...
            normalized_tag = [
                tagname.lower().capitalize() for tagname in tag.split("+")
            ]
        ...
        if pos == "Der":
            derivation_types = {
                ...
                "A-DER-V": parse_tag("A+Der/AV+V+Ind+Person-Number-ConNeg"),
        ...
        if self.settings.get("case") == "N-REVDIM":
            ...
            # if possessive == '':
            TAG_QUERY = TAG_QUERY & Q(
                number__in=number, possessive=possessive, derivation=derivation
            )
            # else:
        ...
        if pos == "Pron":
            sylls = False
            TAG_QUERY = (
                TAG_QUERY & Q(subclass=pron_type) & Q(number__in=["", "Pl", "Sg"])
            )  # Sg added by Heli, Du removed by Pavel
        elif pos2 == "Num":
            sylls = False
        ...
        if tags.count() == 0:
            ...
            error = (
                "Morfa.get_db_info: Database is improperly loaded.\
        ...
        try:
            ...
            while no_form and count < 10:
                ...
                random_word = tag.form_set.filter(
                    WORD_FILTER, SOURCE_FILTER, word__language=LLL1
            ...
            if 0 in [tc, wc, fc, wfc, tfc]:
                # print error
                error += (
                    "Word count (%d), Tag count (%d), Form count (%d), Words matching query (%d), Tags matching query (%d)."
                    % (wc, tc, fc, wfc, tfc)
                )
                error += "\n  Query: %s" % QUERY
    ...
    def create_form(self, db_info, n, data=None):
        ...
        translations = sum(
            [w.word_answers for w in word.translations2(target_key).all()], []
        )
...
class NumGame(Game):
    ...
    def check_answer(self, question, useranswer, formanswer):
        ...
        if useranswer.strip():
            ...
            try:
                error_fst = feedback_api.get_all_feedback_for_form(
                    useranswer.strip(), **message_kwargs
                )
            ...
            if gametype == "string":
                ...
                if useranswer in [a[0] for a in num_list] and question in [
                    a[1] for a in num_list
                ]:
                ...
                try:
                    ...
                    pass
                if question in [a[1] for a in num_list] or useranswer in num_list:
                    is_corr = True
                    if error_fst:
...
class QuizzGame(Game):
    ...
    def get_db_info(self, db_info):
        ...
        frequency = (
            True and self.settings["frequency"] or False
        )  # frequency value or False
        ...
        error = (
            "QuizzGame.get_db_info: Database may be improperly loaded. \
		Query for %s-%s, semantic type %s and book %s returned zero results."
            % ((source_language, target_language, semtypes, source))
        )
        ...
        if not self.query_set:
            leksa_kwargs = {"lang": source_language, "tx_lang": target_language}
