...
class Gameview(object):
    ...
    def register_logs(self, request, game, settings_form):
        ...
        request.user_logs_generated = [
            f.last_log for f in game.form_list if hasattr(f, "last_log")
        ]
        ...
        request.session["set_completed"] = request.session[
            "all_correct"
        ] or game.show_correct in [1, True, "True", "true"]
    ...
    def create_deeplink(self, game, settings_form):
        ...
        if key_filter:
            key_values = [
                "%s=%s" % (k, v) for k, v in values.items() if k in key_filter
            ]
        else:
    ...
    def create_game(self, request, **init_kwargs):
        ...
        settings, settings_form, is_new_game = self.getSettingsForm(
            request, initial_kwargs=init_kwargs
        )
...
class Morfaview(Gameview):
    gamenames = {
        ...
        "V-TA-FUT-DEF": _(
            "Practice Transitive Animate (TA) verbs in future definite form."
        ),
        "V-TA-FUT-INT": _(
            "Practice Transitive Animate (TA) verbs in future intentional form."
        ),
        ...
        "V-TI-FUT-DEF": _(
            "Practice Transitive Inanimate (TI) in future definite form."
        ),
        "V-TI-FUT-INT": _(
            "Practice Transitive Inanimate (TI) in future intentional form."
        ),
        "V-TA-CNJ-PRS": _(
            "Practice Transitive Animate (TA) Conjunct in present tense."
        ),
        "V-TA-CNJ-PRT": _("Practice Transitive Animate (TA) Conjunct in past tense."),
        "V-TA-CNJ-FUT-INT": _(
            "Practice Transitive Animate (TA) Conjunct future intentional form."
        ),
        "V-AI-CNJ-PRS": _(
            "Practice Animate Intransitive (AI) Conjunct in present tense."
        ),
        "V-AI-CNJ-PRT": _("Practice Animate Intransitive (AI) Conjunct in past tense."),
        "V-AI-CNJ-FUT-INT": _(
            "Practice Animate Intransitive (AI) Conjunct in future intentional form."
        ),
        "V-AI-FUT-INT": _(
            "Practice Animate Intransitive (AI) verbs in the future intentional form."
        ),
        "V-AI-FUT-DEF": _(
            "Practice Animate Intransitive (AI) verbs in the future definite form."
        ),
        ...
        "V-TI-CNJ-FUT-INT": _(
            "Practice Transitive Inanimate (TI) Conjunct in future intentional form."
        ),
        ...
        if self.settings["pos"] == "Num":
            if self.settings["gametype"] == "bare":
                if not self.settings["num_type"] or self.settings["num_type"] == "CARD":
                    ...
                    gamename_key = "%s-%s" % (
                        self.settings["num_type"],
                        self.settings["num_bare"],
                    )
...
class Sahkaview(Cealkkaview):
    ...
    def set_gamename(self):
        if "gamename_key" not in self.settings:
            game_name = " - ".join(
                [self.settings["gametype"], self.settings["dialogue"]]
            )
    ...
    def create_game(self, request, **init_kwargs):
        ...
        if is_new_game:
            ...
            if game.form_list[game.num_fields - 2].error == "correct":
                game.update_game(
                    len(game.form_list) + 1, game.form_list[game.num_fields - 2]
                )
                game.settings["attempts"] = 0
            else:
