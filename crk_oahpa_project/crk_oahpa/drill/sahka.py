...
class SahkaGame(Game):
    ...
    def update_game(self, counter, prev_form=None):
        ...
        if (
            Topic.objects.filter(
                Q(dialogue__name=self.settings["dialogue"])
        ...
        if (
            prev_form
            and prev_utttype == "opening"
            and topic.utterance_set.filter(utttype="question").count() > 0
        ):
    ...
    def create_form(self, db_info, n, data=None):
        ...
        if utterance.links.filter(~Q(target="")):
            ...
            for t in target_els:
                targets.append(force_str(t.target))
