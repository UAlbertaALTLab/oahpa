...
class CealkkaGame(Game):
    ...
    def get_qword(self, qelement, tag_el):
        ...
        word = Word.objects.filter(wordqelement__qelement=qelement, form__tag=tag_el.id)
        if word.count() > 0:
            ...
            possible_words = Word.objects.filter(
                wordqelement__qelement=qelement,
        ...
        form_set_filter_dial = form_set_filter.exclude(dialects__dialect="NG").filter(
            dialects__dialect=dialect
        )
        ...
        if word and form_set_filter.count() > 0:
            ...
            raise Form.DoesNotExist("No words found.")
        # return None
                ...
                if element.agreement:
                    ...
                    if agr_syntax in qwords:
                        ...
                        if "tag" in qword:
                            ...
                            tag_count = element.tags.filter(
                                Q(personnumber=anumber) | Q(number=anumber)
                            ).count()
                            if tag_count > 0:
                                tag_el = element.tags.filter(
                                    Q(personnumber=anumber) | Q(number=anumber)
    ...
    def generate_answers_mainv(self, answer, question, awords, qwords):
        ...
        if not mainv_words and "MAINV" in qwords:
        ...
        if element.agreement:
            ...
            if agr_syntax in qwords:
                ...
                if "tag" in qword:
                    ...
                    if tag_count > 0:
                        tag_elements = element.tags.filter(
                            Q(personnumber=anumber) | Q(number=anumber)
                        )
    ...
    def get_question_cealkka(self, db_info, qtype):
        ...
        qwords = {}
        if "level" in self.settings and self.settings["level"] not in ["All", "all"]:
        ...
        #   lemmacount=2
        question = (
            Question.objects.filter(gametype="cealkka", level__lte=level)
            .annotate(ac=Count("answer_set"))
            .filter(ac__gt=0)
            .order_by("?")[0]
        )
        # print level
    ...
    def get_answer_morfa(self, db_info, question):
        ...
        if "answer_id" in db_info and self.settings["gametype"] == "cealkka":
