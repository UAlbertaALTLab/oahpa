...
def aggregate_grades(sender, **kwargs):
    ...
    if game_count > 0:
        stats = grades.aggregate(
            grade_max=Max("score"), grade_min=Min("score"), grade_avg=Avg("score")
        )
...
def create_profile(sender, **kwargs):
    ...
    new_oid, created = OpenID.objects.get_or_create(
        user=user_obj, openid=user_obj.username, default=True
    )
...
def course_relationship_postsave(sender, **kwargs):
    ...
    instructors = course.courserelationship_set.filter(
