...
class Paradigm:
            ...
            for line in tags:
                if line.startswith("#"):
                    ...
                    if add_db and tagclass and string:
                        ...
                        tagset, created = sdm.Tagset.objects.get_or_create(
                            tagset=tagclass
                        )
                        pos, created = sdm.Tagname.objects.get_or_create(
                            tagname=string, tagset=tagset
                        )
                        print("%s added to %s" % (string, tagclass))
        else:
            tagname_tagset = sdm.Tagname.objects.all().values_list(
                "tagname", "tagset__tagset"
            )
            tagset_dict = dict()
    ...
    def read_paradigms(self, paradigmfile, tagfile, add_database, pos):
        ...
        while True:
            ...
            try:
                ...
                print(
                    " * Could not match pos. Check format of paradigm file.",
                    file=STDERR,
                )
    ...
    def get_paradigm(self, lemma, pos, forms, dialect=False, wordtype=None):
        ...
        try:
            ...
            print(
                "No forms generated for %s in dialect %s"
                % (lemma.encode("utf-8"), dialect.encode("utf-8")),
                file=STDERR,
            )
            lines_tmp = False
            if not forms:
        ...
        if forms:
            if forms.getElementsByTagName("form"):
                ...
                for f in form_els:
                    ...
                    print(
                        "adding extra wordform..", wordform.encode("utf-8"), file=STDOUT
                    )
        # HIDCHANGES
        if lines_tmp:
    ...
    def generate_numerals(self):
        ...
        for num in range(1, 20):
            ...
            w, created = sdm.Word.objects.get_or_create(
                wordid=num, lemma=numstring, pos="Num"
            )
            w.save()
            ...
            for form in self.paradigm:
                ...
                g = form.classes
                t, created = sdm.Tag.objects.get_or_create(
                    string=form.tags,
                    pos=g.get("Wordclass", ""),
                    number=g.get("Number", ""),
                    case=g.get("Case", ""),
                ...
                t.save()
                form, created = sdm.Form.objects.get_or_create(
                    fullform=form.form, tag=t, word=w
                )
                form.save()
