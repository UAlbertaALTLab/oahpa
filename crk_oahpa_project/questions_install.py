...
class Questions:
    ...
    def read_element(self, qaelement, el, el_id, qtype):
        ...
        qelems = sdm.QElement.objects.filter(
            question__id=qaelement.question_id, identifier=content_id
        )

        if (not el or el.getAttribute("content")) and sdm.QElement.objects.filter(
            question__id=qaelement.question_id, identifier=content_id
        ).count() > 0:
            question_qelements = sdm.QElement.objects.filter(
                question__id=qaelement.question_id, identifier=content_id
            )
        else:
            if el and el.getAttribute("content"):
                if (
                    sdm.QElement.objects.filter(
                        question__id=qaelement.id, identifier=content_id
                    ).count()
                    > 0
                ):
                    question_qelements = sdm.QElement.objects.filter(
        ...
        if not el and question_qelements:
            for q in question_qelements:
                qe = sdm.QElement.objects.create(
                    question=qaelement,
                    identifier=el_id,
                    syntax=q.syntax,
                    gametype=qaelement.gametype,
                )  # added by Heli
        ...
        if syntax == "MAINV" or syntax == "MAINVOBJ":
            ...
            if (
                ...
                agr_elements = sdm.QElement.objects.filter(
                    question=qaelement,
                    syntax__in=agr_id,
                    question__qatype=qaelement.qatype,
                )
        ...
        if agreement:
            print("\tAgreement:", agreement[0].getAttribute("id"))
        ...
        if agreement:
            ...
            if (
                sdm.QElement.objects.filter(
                    question=qaelement, syntax=agr_id, question__qatype=qaelement.qatype
                ).count()
                > 0
            ):
                agr_elements = sdm.QElement.objects.filter(
                ...
                if sdm.Question.objects.filter(id=qaelement.question_id).count() > 0:
                    ...
                    if (
                        sdm.QElement.objects.filter(
                            question__id=qaelement.question_id, syntax=agr_id
                        ).count()
                        > 0
                    ):
                        agr_elements = sdm.QElement.objects.filter(
        ...
        for i in ids:
            ...
            if word_id:
                if word_id_hid:
                    ...
                    word_elements = sdm.Word.objects.filter(
                        wordid=word_id, hid=int(word_id_hid)
                    )
                else:
        ...
        if not word_elements:
            ...
            if el:
                semclasses = [
                    s.getAttribute("class") for s in el.getElementsByTagName("sem")
                ]
                if semclasses:
                    word_elements = sdm.Word.objects.filter(
                        semtype__semtype__in=semclasses
                    )
            elif qaelement.question:
                # check question for copy, grab semclasses
                has_copies = sdm.QElement.objects.filter(
                    question=qaelement.question, identifier=el_id
                )
                if has_copies:
                    semclasses = list(
                        has_copies.values_list("semtype__semtype", flat=True)
                    )
                    word_elements = sdm.Word.objects.filter(
                        semtype__semtype__in=semclasses
                    )
        ...
        if not el or not grammars:
            ...
            preceding = sdm.QElement.objects.filter(
                question=qaelement,
                identifier=el_id,
            )
        ...
        # An element for each different grammatical specification.
            ...
            if poses:
                if el_id in self.grammar_defaults:
                    if self.grammar_defaults[el_id].tags:
                        tagelements = self.grammar_defaults[el_id].tags.filter(
                            pos__in=poses
                        )
            if tags:
                tagstrings = self.get_tagvalues(tags)
                if tagelements:
                    tagelements = tagelements or sdm.Tag.objects.filter(
                        string__in=tagstrings
                    )
                else:
            ...
            for t in tagelements:
                if t.pos == "Pron":
                    if "Pron" not in words:
                        break
                    found = False
                    for w in words["Pron"][:]:
                        corresponding_forms = sdm.Form.objects.filter(
                            tag__in=tagelements, word=w
                        )
            ...
            if len(grammars) > 0:
                additional_messages = OrderedDict(
                    [
                        ...
                        ("<grammar /> specified", [a[1] for a in not_found]),
                        ("Tags specified, check tags.txt for all parts", tags),
                    ]
                )
        ...
        print("\t\tpos: " + ", ".join(list(posvalues.keys())))
        # Add an element for each pos:
        for p in list(posvalues.keys()):
            qe = sdm.QElement.objects.create(
                question=qaelement, identifier=el_id, syntax=syntax
            )
            if semclasses:
                for s in semclasses:
                    semty, _ = sdm.Semtype.objects.get_or_create(semtype=s)
            ...
            if p not in words:
                ...
                if semclasses:
                    ...
                    word_pks = sdm.Word.objects.filter(
                        form__tag__in=qe.tags.all()
                    ).values_list("pk", flat=True)
    ...
    def read_questions(self, infile, grammarfile):
        ...
        self.source_language = (
            qs.getAttribute("source_language") or default_source_language
        )
        if not gametype:
            gametype = "morfa"
        ...
        for q in tree.getElementsByTagName("q"):
            ...
            question_element, created = sdm.Question.objects.get_or_create(
                qid=qid,
                level=int(level),
                lemmacount=int(lemmacount),
                string=text,
                qtype=qtype,
                gametype=gametype,
                qatype="question",
            ...
            if q.getElementsByTagName("sources"):
                ...
                for el in elements:
                    ...
                    if book:
                        ...
                        book_entry, created = sdm.Source.objects.get_or_create(
                            name=book
                        )
                        if created:
            ...
            for ans in answers:
                ...
                answer_element = sdm.Question.objects.create(
                    string=text,
                    qatype="answer",
    ...
    def read_grammar_defaults(self, infile):
        ...
        class GrammarDefault(object):
            ...
            def __init__(self, poses=False, tags=False, words=False, syntax=False):
        ...
        for el in elements:
            ...
            if len(tagstrings) > 0:
                ...
                if tags.count() == 0:
                    ...
                    raise GrammarDefault.Error(
                        element=tag_elements, tagstrings=tagstrings
                    )
                else:
