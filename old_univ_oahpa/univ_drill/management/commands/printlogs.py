...
def printLogs(queryset, csv=False, delimiter=False, attrs=False):
	...
	_fmt = lambda x: [force_text(a).encode('utf-8') for a in 
						x.outputEntry(**pkwargs).split(pkwargs['delimiter'])]  # added force_text() because there seemed to be a decoding problem
